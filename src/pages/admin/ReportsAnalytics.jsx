        ...
        <div className="space-y-6">
            <div className="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p className="text-secondary-500">Insights and data visualization</p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {reports.map((report, index) => (
                    <div key={index} className="bg-white p-6 rounded-xl shadow-sm border border-secondary-200">
                        ...
                        <h3 className="font-bold text-secondary-900 mb-1">{report.title}</h3>
                        <p className="text-sm text-secondary-500 mb-4">{report.type} • {report.date}</p>
                        <button className="w-full py-2 border border-secondary-200 rounded-lg text-secondary-600 text-sm font-medium hover:bg-secondary-50 flex items-center justify-center gap-2">
                            <Download size={16} />
                            Export PDF
                        </button>
                    </div>
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border border-secondary-200">
                <div className="flex justify-between items-center mb-6">
                    ...
                    <div className="flex gap-2">
                        <button className="p-2 hover:bg-secondary-100 rounded text-secondary-500">
                            <Calendar size={20} />
                        </button>
                        <button className="p-2 hover:bg-secondary-100 rounded text-secondary-500">
                            <Download size={20} />
                ...
                <div className="h-64 flex items-end justify-between gap-4 px-4">
                    {[40, 65, 55, 80, 70, 90, 85].map((height, i) => (
                        <div key={i} className="w-full bg-primary-100 rounded-t-lg relative group">
                            <div
                                className="absolute bottom-0 w-full bg-primary-600 rounded-t-lg transition-all duration-500 group-hover:bg-primary-700"
                                style={{ height: `${height}%` }}
                            ></div>
                            <div className="absolute -top-8 left-1/2 -translate-x-1/2 bg-secondary-900 text-white text-xs py-1 px-2 rounded opacity-0 group-hover:opacity-100 transition-opacity">
                                {height * 10} Students
                            </div>
                        </div>
                    ))}
                </div>
                <div className="flex justify-between mt-4 text-sm text-secondary-500 px-4">
                    <span>2018</span>
                    <span>2019</span>
                    <span>2020</span>
                    <span>2021</span>
